...
class Dataset_WISE(Dataset):
    ...
    def __read_data__(self):
        ...
        test_data = np.load(os.path.join(self.root_path, 'X_np_W1_benchmark.npy'))

        #figure out what the shape of test_data is and figure out how to make my pandas dataframe that shape.

        # flattening all the rows of the original array and keeping the last dimension intact.
        ...
        if self.scale:
            #self.scaler.fit(train_data_reshaped)  #JK switching away from this
            self.scaler.fit(test_data_reshaped)  #let the code do this, keep it
        ...
        if self.set_type == 0:  # TRAIN
            ...
            #self.data_x = train_x
            #self.data_y = train_y
            print('not worrying about training')
        elif self.set_type == 1:  # VAL
            #val_x, val_y = self.make_full_x_y_data(val_scaled_orig_shape)
            #self.data_x = val_x
            #self.data_y = val_y
            print('not worrying about validation')
    ...
    def make_full_x_y_data(self, array):
        """  from bard:
  This function prepares full sequences of data for training or prediction from a provided array.

  Args:
      array (numpy.ndarray): A 3-dimensional NumPy array containing the data. The expected shape is
          (num_instances, sequence_length, feature_dimensions).

  Returns:
      tuple: A tuple containing two lists:
          - data_x: A list of NumPy arrays, each representing a single sequence for the model input.
          - data_y: A list of NumPy arrays, each representing the corresponding target sequence for the model.

  Raises:
      ValueError: If the provided array has an invalid shape (not 3-dimensional).
  """
        data_x = []
        data_y = []
